...
\textbf{scipy.optimize:} biblioteca de otimização científica, será utilizada para implementar a carteira de Markowitz através do algoritmo SLSQP (Sequential Least Squares Programming). A ferramenta oferece algoritmos robustos para problemas de otimização com restrições, amplamente utilizada em finanças quantitativas.
...
\subsection{Fórmula 1 -- Equal Risk Contribution (ERC) na Estratégia Risk Parity}
...
Em que:
\begin{itemize}
    \item $RC_i$: contribuição marginal de risco do ativo $i$;
    \item $w_i$: peso do ativo $i$ na carteira;
    \item $\Sigma$: matriz de covariância dos ativos;
    \item $\sigma_p$: volatilidade total da carteira;
    \item $n$: número total de ativos.
\end{itemize}

\textbf{Interpretação:} a formulação ERC garante que cada ativo contribua igualmente ($\frac{\sigma_p}{n}$) para o risco total da carteira, considerando explicitamente as correlações entre os ativos através da matriz de covariância.
...
A série oficial do Ibovespa utiliza retornos mensais calculados por diferença logarítmica de preços de fechamento dos dados diários da B3, representando fielmente o comportamento do mercado brasileiro durante o período 2018-2019. Janeiro de 2018 representa o primeiro mês de operação, com retornos calculados versus preços de dezembro de 2017 tanto para o benchmark quanto para as carteiras analisadas, produzindo retornos reais (não-zero) para este período inicial.

\subsection{Taxa Livre de Risco e Cálculo do Sortino}

Em todas as métricas de avaliação utilizadas neste trabalho, emprega-se a taxa livre de risco de \textbf{6{,}24\% ao ano} (CDI médio mensal de 0,52\%), com base na média do CDI no biênio 2018–2019, aplicada na forma mensal ($r_f/12$) para o cômputo do Índice de Sharpe e do Sortino Ratio. 
